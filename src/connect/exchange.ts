...
export class OKXExchange {
  ...
  async initAccountSettings(symbol: string) {
    try {
      ...
      try {
        ...
        // 忽略 "无需更改" 的错误
        if (
          e.message.includes("not need to be changed") ||
          e.message.includes("80012")
        ) {
   ...
   * 获取未完成的策略委托单
   * @param instId 产品 ID
   * @param ordType 订单类型 (可选)
   */
  async getPendingAlgoOrders(
    instId?: string,
    ordType?: string,
  ): Promise<any[]> {
    try {
      ...
      // OKX API: /api/v5/trade/orders-algo-pending
      const data = await this.request(
        "GET",
      ...
      logger.error("获取未完成策略委托失败:", error);
      throw error;
    }
  }
   ...
   * @param params 修改参数
   */
  async amendAlgoOrder(params: any): Promise<any> {
    try {
      // OKX API: /api/v5/trade/amend-algos
      const data = await this.request(
        "POST",
